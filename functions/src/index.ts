...
export const eventWritten = onDocumentWritten(
  "events/{docId}",
  async (event) => {
    ...
    if (
      beforeData &&
      beforeData.attendees.length < afterData.attendees.length
    ) {
      logger.info("Someone joined an event");
      attendee = afterData.attendees.filter(
        (a) => !beforeData.attendees.some((b) => b.id === a.id)
      )[0];
      status = "attending";
    ...
    if (
      beforeData &&
      afterData.attendees.length < beforeData.attendees.length
    ) {
      logger.info("Someone left event");
      attendee = beforeData.attendees.filter(
        (a) => !afterData.attendees.some((b) => b.id === a.id)
      )[0];
      status = "removed";
      logger.info(attendee, status, event.time);
    }
...
if (attendee) {
  try {
    const followersDocs = await db collection(`profiles/${attendee.id}/followers`).get();
    followersDocs.docs.forEach(async doc => {
      logger.info(doc)
